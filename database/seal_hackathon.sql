...
-- MySQL DDL Script
-- 16 tables and 31 foreign keys
-- =====================================================

CREATE DATABASE IF NOT EXISTS seal_hackathon
...
CREATE TABLE Round (
  ...
  top_n_advance       INT           COMMENT 'Top N teams per track advance to next round',
  is_calibration      BOOLEAN       NOT NULL DEFAULT FALSE COMMENT 'For RBL: judge calibration round',
...
-- =====================================================
-- USER EVENT ROLE (after Event, Track, Round)
-- =====================================================
...
CREATE TABLE UserEventRole (
  ...
  event_id    INT         COMMENT 'Nullable for system-wide roles like ADMIN',
  track_id    INT         COMMENT 'Optional scope: Mentor/Judge per track',
  round_id    INT         COMMENT 'Optional scope: Judge per round',
  judge_type  VARCHAR(20) COMMENT 'INTERNAL or GUEST, only when role = JUDGE',
...
CREATE TABLE ScoringCriteria (
  ...
  event_id     INT           COMMENT 'Null if criteria belongs to template only',
  round_id     INT           COMMENT 'Optional: criteria specific to a round',
...
CREATE TABLE Prize (
  ...
  event_id    INT           NOT NULL,
  track_id    INT           COMMENT 'Null if prize is event-wide',
