...
- **Backend compile:** `mvnw compile` (offline) — `BUILD SUCCESS`, no errors (one pre-existing-pattern deprecation warning on `org.springframework.lang.NonNull`, consistent with other filters already in the codebase — not a regression).
...
## 8. Explicitly out of scope (by user's own choice, §2)
...
## 9. Post-implementation housekeeping
