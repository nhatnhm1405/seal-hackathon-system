                ...
                return (
                  <React.Fragment key={t.team_id}>
                    <tr onClick={() => setExpandedTeamId(expanded ? null : t.team_id)}
                      ...
                      <td style={{ color: C.textMuted, fontSize: 12, padding: "12px 14px" }}>{track?.name}</td>
                      <td style={{ color: C.textMuted, fontSize: 12, padding: "12px 14px" }}>{ev?.name}</td>
                              ...
                              {mems.map(m => {
                                ...
                                return (
                                  <div key={m.user_id} style={{ color: C.textMuted, fontFamily: "'JetBrains Mono', monospace", fontSize: 11, padding: "4px 0" }}>
                                    {u?.full_name} {m.member_role === 'LEADER' && <PixelBadge color="cyan">LEADER</PixelBadge>}
                                  </div>
                                );
                              })}
                            </div>
