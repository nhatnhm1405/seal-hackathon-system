...
import {
    userEventRoles, rounds, submissions, scores, scoringCriteria,
...
export function JudgeDashboard() {
    ...
    const myAssignments = userEventRoles.filter(j => j.role_id === 3 && j.user_id === currentUser.user_id);
    ...
    const roundStats = myRounds.map(round => {
        ...
        const scoredCount = roundSubs.filter(sub => {
            ...
            const criteriaCount = scoringCriteria.length;
                ...
                {roundStats.map(({ round, total, scored }) => (
                    <PixelCard key={round.round_id} glow glowColor="blue" style={{ padding: 20 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                            ...
                            <PixelBadge color={round.status === 'ACTIVE' ? 'green' : round.status === 'PENDING' ? 'yellow' : 'red'}>
                                {round.status}
                            </PixelBadge>
                        </div>
